...
namespace NUnit.Engine.Services
{
    public class RuntimeFrameworkService : Service, IRuntimeFrameworkService, IAvailableRuntimes
    {
        ...
        private RuntimeFramework SelectRuntimeFrameworkInner(TestPackage package)
        {
            ...
            string frameworkSetting = package.GetSetting(PackageSetting.RequestedRuntimeFramework.Name, string.Empty);
            ...
            if (frameworkSetting.Length > 0)
            {
                ...
                package.Settings[PackageSetting.RequestedFrameworkName.Name] =
                    PackageSetting.RequestedFrameworkName.WithValue(frameworkName);
                package.Settings.Add(
                    PackageSetting.TargetFrameworkName.Name,
                    PackageSetting.TargetFrameworkName.WithValue(frameworkName));
            ...
            string imageTargetFrameworkNameSetting = package.GetSetting(PackageSetting.ImageTargetFrameworkName.Name, string.Empty);
            ...
            if (string.IsNullOrEmpty(imageTargetFrameworkNameSetting))
            {
                ...
                var trialVersion = new Version(package.GetSetting(PackageSetting.ImageRuntimeVersion.Name, "2.0"));
            ...
            package.Settings[PackageSetting.TargetFrameworkName.Name] =
                PackageSetting.TargetFrameworkName.WithValue(targetFramework.FrameworkName.ToString());
        ...
        private static void ApplyImageData(TestPackage package)
        {
            ...
            if (package.SubPackages.Count > 0)
            {
                foreach (var subPackage in package.SubPackages)
                {
                    ...
                    Version v = new Version(subPackage.GetSetting(PackageSetting.ImageRuntimeVersion.Name, "0.0"));
                    if (v > targetVersion)
                        targetVersion = v;

                    // Collect highest framework name
                    // TODO: This assumes lexical ordering is valid - check it
                    string fn = subPackage.GetSetting(PackageSetting.ImageTargetFrameworkName.Name, string.Empty);
                    if (fn != string.Empty)
                    {
                    ...
                    if (subPackage.GetSetting(PackageSetting.ImageRequiresX86.Name, false))
                        requiresX86 = true;

                    if (subPackage.GetSetting(PackageSetting.ImageRequiresDefaultAppDomainAssemblyResolver.Name, false))
            ...
            if (targetVersion.Major > 0)
                package.Settings[PackageSetting.ImageRuntimeVersion.Name] =
                    PackageSetting.ImageRuntimeVersion.WithValue(targetVersion.ToString());

            if (!string.IsNullOrEmpty(frameworkName))
                package.Settings[PackageSetting.ImageTargetFrameworkName.Name] =
                    PackageSetting.ImageTargetFrameworkName.WithValue(frameworkName);

            // TODO: Why both?
            package.Settings[PackageSetting.ImageRequiresX86.Name] =
                PackageSetting.ImageRequiresX86.WithValue(requiresX86);
            if (requiresX86)
                package.Settings[PackageSetting.RunAsX86.Name] =
                    PackageSetting.RunAsX86.WithValue(true);

            package.Settings[PackageSetting.ImageRequiresDefaultAppDomainAssemblyResolver.Name] =
                PackageSetting.ImageRequiresDefaultAppDomainAssemblyResolver.WithValue(requiresAssemblyResolver);
        }
    }
}
#endif