...
#pragma warning restore 414
...
#else
            // Attempt to load the .NET 6.0 version of the extensions from the .NET 3.5 tests
            var assemblyName = Path.Combine(GetSiblingDirectory("net6.0"), "nunit.engine.core.tests.dll");
#endif
        ...
        public static IEnumerable<TestCaseData> InvalidTargetFrameworkCombos()
        {
            ...
            var extNetStandard = new ExtensionAssembly(Path.Combine(GetSiblingDirectory("netstandard2.0"), "nunit.engine.dll"), false);
            var extNetCore = new ExtensionAssembly(Path.Combine(GetSiblingDirectory("net6.0"), "nunit.engine.tests.dll"), false);
