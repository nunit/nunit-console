...
{
    // The checks in this class basically do no more than what the programmer might 
    ...
    static readonly string[] ENGINE_FILES = { "nunit.engine.dll", "nunit.engine.core.dll", "nunit.engine.api.dll", "testcentric.engine.metadata.dll" };
    ...
    static readonly string[] CONSOLE_FILES = { "nunit3-console.exe", "nunit3-console.exe.config" };
    static readonly string[] CONSOLE_FILES_NETCORE = { "nunit3-console.exe", "nunit3-console.dll", "nunit3-console.dll.config" };

    private BuildParameters _parameters;
    private ICakeContext _context;

    public PackageChecker(BuildParameters parameters)
    {
    ...
    {
        string packageDir = _parameters.PackageDirectory;
    ...
    {
        return CheckPackage($"{packageId}.{_parameters.PackageVersion}.nupkg", checks);
    }
    ...
    {
        return CheckPackage($"{packageName}-{_parameters.PackageVersion}.zip", checks);
    }
    ...
    private bool CheckPackage(string fileName, params ICheck[] checks)
    {
        ...
        string package = _parameters.PackageDirectory + fileName;
        ...
        {
            _context.DeleteDirectory(tempDir, new DeleteDirectorySettings()
            {
                Recursive = true,
                Force = true
            });
    ...
    private string InstallMsiToTempDir(string package)
    {
        ...
        int rc = _context.StartProcess("msiexec", $"/a {package} TARGETDIR={tempDir} /q");
        if (rc != 0)
    ...
    private string UnzipToTempDir(string package)
    {
        ...
        _context.Unzip(package, tempDir);
